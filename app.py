...
st.title("🧮 Công Cụ Tính Thuế Thu Nhập Cá Nhân (TNCN)")
...
# Thu nhập chịu thuế = Gross - Bảo hiểm - Giảm trừ khác
thu_nhap_chiu_thue = max(0, luong_gross - bao_hiem - giam_tru_khac)

# Thu nhập tính thuế = Thu nhập chịu thuế - Giảm trừ gia cảnh
thu_nhap_tinh_thue = max(0, luong_gross - tong_giam_tru)

thue_phai_nop, chi_tiet_thue = tinh_thue_tncn(thu_nhap_tinh_thue)
luong_net = luong_gross - bao_hiem - thue_phai_nop
...
res_col1.metric("Lương NET thực nhận", f"{luong_net:,.0f} VNĐ")
res_col2.metric("Thuế TNCN phải nộp", f"{thue_phai_nop:,.0f} VNĐ")
res_col3.metric("Tổng giảm trừ", f"{tong_giam_tru:,.0f} VNĐ")

# Bảng diễn giải chi tiết
with st.expander("📋 Xem chi tiết diễn giải tính toán"):
    st.write(f"- **Thu nhập GROSS:** {luong_gross:,.0f} VNĐ")
    st.write(f"- **Các khoản bảo hiểm:** -{bao_hiem:,.0f} VNĐ")
    st.write(f"- **Giảm trừ bản thân:** -{GIAM_TRU_BAN_THAN:,.0f} VNĐ")
    st.write(f"- **Giảm trừ người phụ thuộc ({nguoi_phu_thuoc} người):** -{tong_giam_tru_phu_thuoc:,.0f} VNĐ")
    if giam_tru_khac > 0:
        st.write(f"- **Giảm trừ khác:** -{giam_tru_khac:,.0f} VNĐ")
    st.write(f"👉 **Thu nhập tính thuế:** {thu_nhap_tinh_thue:,.0f} VNĐ")

    if chi_tiet_thue:
        st.markdown("#### Diễn giải tiền thuế theo từng bậc:")
        for ten_bac, phan_chieu_thue, thue_bac_nay in chi_tiet_thue:
            st.write(f"+ {ten_bac}: **{phan_chieu_thue:,.0f}** VNĐ × thuế suất = **{thue_bac_nay:,.0f}** VNĐ")
    else:
        st.success("Thu nhập của bạn chưa đến mức phải nộp thuế TNCN!")
